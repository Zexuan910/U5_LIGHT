...
static void LCD_1IN69_WriteRepeatedColor(UWORD Color, UDOUBLE PixelCount)
{
    ...
    UWORD i;
    UBYTE ColorHi = Color >> 8;
    UBYTE ColorLo = Color & 0xff;

    for (i = 0; i < LCD_1IN69_FILL_BLOCK_PIXELS; i++) {
        Buffer[i * 2U] = ColorHi;
...
void LCD_1IN69_FillRect_FastStatic(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend, UWORD Color)
{
    ...
    if (Xend >= LCD_1IN69.WIDTH) {
        Xend = LCD_1IN69.WIDTH - 1U;
    }

    if (Yend >= LCD_1IN69.HEIGHT) {
        Yend = LCD_1IN69.HEIGHT - 1U;
    }
...
{
    static UBYTE Buffer[LCD_1IN69_FILL_BLOCK_PIXELS * 3U];
    UDOUBLE PixelCount;
    UDOUBLE Remaining;
    UWORD i;
    ...
    for (i = 0; i < LCD_1IN69_FILL_BLOCK_PIXELS; i++) {
        Buffer[i * 3U] = Red;
        Buffer[i * 3U + 1U] = Green;
        Buffer[i * 3U + 2U] = Blue;
    }

    PixelCount = (UDOUBLE)(Xend - Xstart + 1U) * (UDOUBLE)(Yend - Ystart + 1U);
    Remaining = PixelCount;

    LCD_1IN69_SetWindows(Xstart, Ystart, (UWORD)(Xend + 1U), (UWORD)(Yend + 1U));
    LCD_1IN69_DC_1;
    LCD_1IN69_CS_0;

    while (Remaining > 0U) {
        UDOUBLE CurrentPixels = Remaining;
        if (CurrentPixels > LCD_1IN69_FILL_BLOCK_PIXELS) {
            CurrentPixels = LCD_1IN69_FILL_BLOCK_PIXELS;
        }

        DEV_SPI_WriteBuffer(Buffer, CurrentPixels * 3U);
...
void LCD_1IN69_DrawColorBars(void)
{
    ...
    UWORD BarWidth = LCD_1IN69.WIDTH / BarCount;
    UWORD Xstart = 0;
    UWORD i;
