...
static void UI_StartExercise(uint32_t now_ms)
{
  ...
  if (sport <= 1U)
  {
    metrics_state = (sport == 1U) ? &run_metrics_state : &walk_metrics_state;
    metrics_output = (sport == 1U) ? &run_metrics_output : &walk_metrics_output;
    mode = (sport == 1U) ? WALK_METRICS_MODE_RUN : WALK_METRICS_MODE_WALK;
